...
#        }
    ...
    /**
     * Checks the presence of one of $templates in $text
     * @param string $text		page text
     * @param array	$templates	array of template names
     * @return boolean
     */
    public static function testTemplates($text, $templates) {
    	if(!empty($templates)) {
    		foreach($templates as $template) {
    			$tpl = new Template($text, strtolower($template));
    			if($tpl)
    				return true;
    			unset($tpl);
    		}
    ...
    /**
     * Return the string used in {@link __construct()} without the template at all
     * 
     * @return string
     */
    ...
    /**
     * Deletes a field with no name or identifier
     * 
     * @param string $fieldname Field to delete
     */
    public function removeanonfield($fieldname) {
    	$id = array_search($fieldname, $this->fields);
    	/* As array_search looks for values, not keys,
    	 * it's checked that key is int.
    	 */
    	if(is_int($id))
