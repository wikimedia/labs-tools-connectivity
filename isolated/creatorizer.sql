...
CREATE PROCEDURE creatorizer ()
  BEGIN
    ...
    SET @st=CONCAT( 'CREATE TABLE creators ( iid int(8) unsigned NOT NULL default ', "'0'", ', user int(8) unsigned NOT NULL default ', "'0'", ', user_text varchar(255) binary NOT NULL default ', "''", ' ) ENGINE=MyISAM AS SELECT iid, rev_user as user, rev_user_text as user_text FROM ', @dbname, '.revision, firstrev WHERE revision=rev_id;' );
    PREPARE stmt FROM @st;
    EXECUTE stmt;
    DEALLOCATE PREPARE stmt;
